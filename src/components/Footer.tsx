      ...
      <div className="container-main px-5 md:px-12 py-12 md:py-20">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-12">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {categories.map((cat) => (
                <li key={cat.slug}>
                  <Link href={`/category/${cat.slug}`} className="text-xs text-white/35 hover:text-white/70 transition-colors">
                    {cat.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              <li><Link href="/about" className="text-xs text-white/35 hover:text-white/70 transition-colors">つづくウェルネスについて</Link></li>
              <li><Link href="/tools/bmi" className="text-xs text-white/35 hover:text-white/70 transition-colors">BMI計算</Link></li>
              <li><Link href="/tools/calorie" className="text-xs text-white/35 hover:text-white/70 transition-colors">カロリー計算</Link></li>
            </ul>
          </div>
        ...
        <div className="mt-16 pt-8 border-t border-dark-border flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            <Link href="/privacy" className="text-[0.6rem] text-white/15 hover:text-white/40 transition-colors">Privacy</Link>
            <Link href="/terms" className="text-[0.6rem] text-white/15 hover:text-white/40 transition-colors">Terms</Link>
          </div>
        </div>
      </div>
    </footer>
