...
export default function NewArticlePage() {
  ...
  const [author, setAuthor] = useState('RESIST トレーナーチーム')
  ...
  useEffect(() => {
    supabase.from('tw_categories').select('*').order('sort_order').then(({ data }) => {
      setCategories((data as Category[]) || [])
      if (data && data.length > 0 && !categorySlug) {
    ...
    <div className="max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-semibold text-gray-900">新しい記事を作成</h1>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">概要（excerpt）</label>
            <textarea
              value={excerpt}
          ...
          {sections.map((section, i) => (
            <div key={i} className="p-4 bg-gray-50 rounded-lg space-y-3">
              ...
              <textarea
                ...
                onChange={(e) => {
                  const s = [...sections]
                  s[i].body = e.target.value
                  setSections(s)
                }}
                rows={6}
                placeholder="本文（段落は空行で区切る）"
